...
#include "arduwire.h"
		...
		arduwire::arduwire(uint8_t test_points){
			
			//CONSTRUCTOR SET THE TOTAL NUMBER OF POINTS
			//CREATE ARRAYS
			num_total_tp = test_points;
			
			
			if(num_total_tp <= 32){
				enbl_pins = 2;//SET THE NUMBER OF THE ENABLE PINS TO 2
			}
			else{
				enbl_pins = 4;//
			}
		...
		void arduwire::set_total_tp(uint8_t tp){
			num_total_tp = tp;
		}		
		...
		void arduwire::learn(){
			...
			for(uint8_t i = 0; i < 2; i++){
				...
				for(uint8_t u = 0; u < 16; u++){
					...
					if((PINB &(1<<PINB4))==0){// I FOUNDED ONE CONNECTION
						...
						num_total_connections_arduwire++;//set the value_comp
			...
			if(num_total_connections_arduwire!=0){
				cable_learned = true;
		...
		uint8_t arduwire::get_connections_numbers(){
			return num_total_connections_arduwire;
		}
		...
		bool arduwire::test(){
				...
				if(memcmp(pina_tested, pina_learned, sizeof(pina_learned))== 0 && memcmp(pinb_tested, pinb_learned, sizeof(pinb_learned))==0){
					return 1;//return 1 it is true all connections it is the same
				}
				else{
